...
func main() {
	...
	movieMS := config.NewApp("movie-service", db, &sync.WaitGroup{})

	// Initialize User Microservice
	movieMS.Init()

	// Register User Microservice
	config.RegisterRoutes(movieMS)

	// Migrate Table
	config.TableMigration(movieMS)

	// Start Microservice
	log.Println("Movie microservice started successfully.")
	err := movieMS.StartServer()
	if err != nil {
		fmt.Println(err)
	}
}
